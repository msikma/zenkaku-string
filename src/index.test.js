...
describe('CJK Length', () => {
  ...
  describe('cjkSlice()', () => {
    describe('string slicing correctly takes wide characters into account when', () => {
      it('contains only non-wide characters', () => {
        expect(cjkSlice('asdf', 2)).toBe('asdf'.slice(2))
      })
      it('contains wide partially inside the slice range', () => {
        expect(cjkSlice('asdfあい asdfai あいasdf', 2, 8)).toBe('dfあい')
        expect(cjkSlice('asdfあい asdfai あいasdf', 2, 12)).toBe('dfあい asd')
        expect(cjkSlice('asdfあい asdfai あいasdf', 1, 3)).toBe('sdf')
      })
    })
  })
})
